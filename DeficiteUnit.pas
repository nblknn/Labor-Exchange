...
Begin
    ...
    While Temp <> Nil Do
    Begin
        I := FindArrPosition(Temp.Title);
        If I = -1 Then
        Begin
            SetLength(TitleArr, Length(TitleArr) + 1);
            TitleArr[High(TitleArr)].Title := Temp.Title;
...
Begin
    ...
    While Temp <> Nil Do
    Begin
        If AnsiUpperCase(Temp.Title) = AnsiUpperCase(Title) Then
        Begin
            AddCandidate(Temp^, Head);
            AddCandidateToListView(Temp^, DeficiteForm.ListView);
        End;
        Temp := Temp.Next;
    End;
End;
